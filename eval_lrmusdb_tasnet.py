...
def eval_main(
    ...
    for track in test_dataset:
        ...
        subprocess.run(["python3", "-m", "demucs.separate", "-n", "tasnet", "-o",
            os.path.join(output_path, track[:-4]), "-f", input_file])

        estimates = {}
        #TODO: Check the rates
        audio, rate = sf.read(os.path.join(output_path, track[:-4], 'vocals.wav'), always_2d=True, start=start, stop=stop)
        estimates['vocals'] = audio
        audio, rate = sf.read(os.path.join(output_path, track[:-4], 'drums.wav'), always_2d=True, start=start, stop=stop)
        estimates['drums'] = audio
        audio, rate = sf.read(os.path.join(output_path, track[:-4], 'bass.wav'), always_2d=True, start=start, stop=stop)
        estimates['bass'] = audio
        audio, rate = sf.read(os.path.join(output_path, track[:-4], 'other.wav'), always_2d=True, start=start, stop=stop)
        estimates['other'] = audio
...
if __name__ == "__main__":
    ...
    with open("cfg/eval_lrmusdb_tasnet.yml") as f:
